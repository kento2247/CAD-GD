...
def collate_fn(batch):
    ...
    return (
        ...
        img_ids,
        density_maps[0],
    )  # tensor (bs,3,h,w), list (), list ((w,h)), list ()
...
def get_loader(processor: DataProcessor, split, batch_size):
    ...
    split_loader = DataLoader(
        split_set, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_fn
    )
...
class Rec8KDataset(Dataset):
    def __init__(self, processor: DataProcessor, split):
        ...
        split_set_tuples = processor.get_img_ids_for_split(
            split
        )  # list of (img_id, cap)
        ...
        self.labels = [
            list(split_dict[img_id]) for img_id in self.img_ids
        ]  # list of list of caps
        # 2245 - 2250
    ...
    def __getitem__(self, idx):
        ...
        img_cap_tuple = self.img_cap_tuples[
            idx
