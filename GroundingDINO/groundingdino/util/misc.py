...
def clean_state_dict_test(state_dict):
...
#             k = k[7:]  # remove `module.`
#         new_state_dict[k] = v
#         if 'transformer.enc_output' in k:
#             name = k[:22] + '_regression' + k[22:]
#             new_state_dict[name] = v.clone()
#     return new_state_dict
...
def clean_state_dict(state_dict):
    ...
    for k, v in state_dict.items():
        ...
        new_state_dict[k] = v
        if 'transformer.enc_output' in k:
            name_regression = k[:22] + '_regression' + k[22:]
            new_state_dict[name_regression] = v.clone()
            
            # name_density = k[:22] + '_density' + k[22:]
            # new_state_dict[name_density] = v.clone()
