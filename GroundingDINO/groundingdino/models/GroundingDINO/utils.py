...
class ContrastiveEmbedBox(nn.Module):
    ...
    def forward(self, x, box_dict): # x: layer_hs (bs, 18259, 256) # hiddens state (fused feature)
        """_summary_

        Args:
            x (_type_): _description_ 
            box_dict (_type_): _description_
            {
                'encoded_box': encoded_text, # bs, 195, d_model
        ...
        """
        assert isinstance(box_dict, dict)

        y = box_dict["encoded_box"] # (bs,40,256)
        text_token_mask = box_dict["box_mask"] # (bs,40)
...
## TODO: find n peak density position  
## input: torch [b, h, w]
def top_n_positions(tensor, n):
    ...
    返回:
        indices: 大小为 [b, n, 2] 的张量，每个位置的索引 (row, col)
    """
    b, h, w = tensor.shape
    ...
    # 对每个密度图获取前 n 大的索引
    top_n_indices = flattened.topk(n, dim=1).indices  # 形状为 [b, n]
    
    # 将一维索引转换为二维索引
    row_indices = top_n_indices // w  # 行索引
    col_indices = top_n_indices % w   # 列索引
    
    # 拼接行和列索引，形状为 [b, n, 2]
    ref_points = torch.stack((row_indices/h, col_indices/w), dim=-1)
...
def find_local_maxima(tensor, neighborhood_size=10):
    ...
    参数:
        tensor: 输入张量，形状为 [b, h, w]
        neighborhood_size: 定义局部邻域的大小，用于寻找局部最大值

    返回:
        local_max_mask: 一个布尔张量，表示局部最大值的位置，形状为 [b, h, w]
    """
    padding = neighborhood_size // 2
    
    # 对输入的密度图进行填充，使其在池化后与输入尺寸一致
