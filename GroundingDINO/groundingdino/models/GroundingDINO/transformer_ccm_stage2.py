...
from .context_density_module import CCM, CGFE, CACCM
from .utils import (
    MLP,
    _get_activation_fn,
    _get_clones,
...
class Transformer(nn.Module):
    def __init__(
        ...
        # setting the ccm
        self.ccm = CCM(in_channels=256, hidden_dim=512, out_dim=512)
        self.cgfe = CGFE(in_channels=512, out_channels=256)
    ...
    def forward(
        ...
        ## CCM module
        ## reconstruct multi-level feature map
        ...
        density_feature, density_cls = self.ccm(cnn[0].detach())
        # density_feature, density_cls = self.ccm(cnn[0].detach())
        cnn_enhanced = self.cgfe(cnn, density_feature)
        memory_flatten = []
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            memory_flatten_ = cnn_enhanced[lvl].view(N_, C_, -1)
            memory_flatten.append(memory_flatten_)
        memory = torch.cat(memory_flatten, dim=-1).permute(0, 2, 1)
        # mean: 16.7 std: 27.78
        ### x<10, 10<x<40, 40<x<140, x>140
        # self.num_queries = torch.argmax(density_cls,dim=1)
        ...
        if self.two_stage_type == "standard":
            ...
            lower_tokens = torch.gather(
                output_memory, 1, lower_idxes.unsqueeze(-1).expand(-1, -1, self.d_model)
            )
            higher_tokens = torch.gather(
                output_memory,
                1,
                higher_idxes.unsqueeze(-1).expand(-1, -1, self.d_model),
            )
            ...
            subject_text_tokens = [
                text[mask]
                for text, mask in zip(text_dict["encoded_text"], text_subject_mask)
            ]
            context_text_tokens = [
                text[mask]
                for text, mask in zip(text_dict["encoded_text"], text_context_mask)
            ]
            ...
            subject_mask = torch.stack(
                [
                    torch.cat(
                        [
                            torch.ones(t.size(0)).to(t.device),
                            torch.zeros(max_size - t.size(0)).to(t.device),
                        ]
                    )
                    for t in subject_text_tokens
                ]
            )
            context_mask = torch.stack(
                [
                    ...
                    for t in context_text_tokens
                ]
            )

            lower_tokens = self.cross_attention(
                lower_tokens, padded_subject_text_tokens, V_mask=subject_mask
            )
            if context_mask.sum() > 2:  # not all [CLS][SEP] tokens
                higher_tokens = self.cross_attention(
                    higher_tokens, padded_context_text_tokens, V_mask=context_mask
                )

            updated_lower_tokens = self.cross_attention(lower_tokens, higher_tokens)
            output_memory = output_memory.scatter(
                1, lower_idxes.unsqueeze(-1).expand(-1, -1, 256), updated_lower_tokens
            )
